    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-0.5">{{ $category->name }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-lg mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-xl border border-gray-100 overflow-hidden">
                ...
                <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="px-6 py-6 space-y-5">
                    @csrf @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                                  class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">{{ old('description', $category->description) }}</textarea>
                        @error('description')
